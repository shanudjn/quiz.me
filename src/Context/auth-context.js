...
export function AuthProvider({ children }) {
  ...
  async function loginService(username, password) {
    const loginServiceResponce = await axios.post(
      "https://quiz-me-backend.onrender.com/user/login",
  ...
  return (
    <AuthContext.Provider
      value={{ isUserLoggedIn, token, loginUserWithCredentials, logOutUser }}
    >
      {children}
    </AuthContext.Provider>
  );
